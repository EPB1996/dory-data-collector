...
async def create_meeting(meeting: Meeting) -> MeetingRead:
    existing_meeting = firestore.get_document_by_field(
        MEETING_COLLECTION, "randomIdentifier", meeting.randomIdentifier
    )
    existing_meeting_id = (
        MeetingRead(**existing_meeting).id if existing_meeting else None
    )

    if not existing_meeting_id:
        update_time, doc_ref = firestore.add_document(
            MEETING_COLLECTION, meeting.model_dump()
        )
        meeting = doc_ref.get().to_dict()
        meeting["id"] = doc_ref.id
    else:
        firestore.update_document(
            MEETING_COLLECTION, existing_meeting_id, meeting.model_dump()
        )
